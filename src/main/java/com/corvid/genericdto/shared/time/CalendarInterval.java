...
public abstract class CalendarInterval extends Interval {
    ...
    /**
     * gives an iterator of all the intervals of subintervalLength
     *
     * @param subintervalLength
     * @return
     */
     ...
     * @return
     */
    public Iterator<CalendarDate> subintervalUpperLimitIterator(Duration subintervalLength) {
        //assert TimeUnit.day.compareTo(subintervalLength.normalizedUnit()) <= 0;
        if (TimeUnit.day.compareTo(subintervalLength.normalizedUnit()) > 0) {
            throw new IllegalArgumentException("CalendarIntervals must be a whole number of days or months.");
        }
